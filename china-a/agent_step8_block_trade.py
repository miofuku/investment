...
def _build_meta(code, results):
    """从工具的确定性返回(非LLM文字)汇总列表展示用的结构化元信息。
    供 push_to_sheets 直接解析,替代从 Markdown 正则抽取(脆弱)。"""
    ...
    block = results.get("get_block_trades") or {}
    ...
    flags = []
    sigs = set()
    for hit in (disc.get("命中明细") or []):
        for s in (hit.get("信号") or []):
            sigs.add(s)
    if {"会计差错", "更正公告"} & sigs:
        flags.append("会计差错")
    if {"立案", "处罚"} & sigs:
        flags.append("监管处罚")
    if {"问询函", "关注函", "监管函", "警示函"} & sigs:
        flags.append("监管问询")
    if roic.get("可信") is False:
        flags.append("ROIC失真")
    sc = cap.get("股东增减持")
    if isinstance(sc, dict) and sc.get("净方向") == "净减持":
        flags.append("产业资本净减持")
    if (block.get("大幅折价笔数") or 0) > 0:
        flags.append("大宗折价")
    for f in (rfd.get("深度红旗") or []):
        flags.append(f)

    return {
        "code": str(code).zfill(6),
        "name": basics.get("name"),
        "industry": basics.get("行业"),
        "pb": _n(basics.get("PB")),
...
def run_agent(code, max_steps=22):
    """返回 (简报正文, meta)。meta 由工具返回确定性汇总,不依赖 LLM 文字。"""
    ...
    for _ in range(max_steps):
        ...
        for tc in msg.tool_calls:
            ...
            if isinstance(result, dict) and "error" not in result:
                tool_results[tc.function.name] = result   # 每个工具留最后一次成功返回
...
if __name__ == "__main__":
    ...
    # 结构化 META 先于正文打印,便于 push_to_sheets 精确解析(不混入简报正文)
